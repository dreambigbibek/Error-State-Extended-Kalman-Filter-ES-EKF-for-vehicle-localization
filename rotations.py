...
def skew_symmetric(v):
    ...
    if v.size != 3:
        raise ValueError(f"skew_symmetric expected 3 elements, got shape {np.shape(v)}")
    x, y, z = v
...
class Quaternion():
    ...
    def to_mat(self):
        v = np.array([self.x, self.y, self.z], dtype=np.float64).reshape(3,)  # 1D
        vv = v @ v  # scalar = x^2 + y^2 + z^2
        return (self.w**2 - vv) * np.eye(3) + 2.0 * np.outer(v, v) + 2.0 * self.w * skew_symmetric(v)
